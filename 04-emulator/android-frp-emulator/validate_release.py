"""Deterministic release gate v4 for the controlled FRP simulator."""
...
def main() -> int:
    ...
    checks = {
        "scope": all(x.get("scope") == "controlled-simulator-only" for x in (evidence, audit, matrix, faults, resilience)),
        ...
        "policy_profile_coverage": audit.get("policy_profile_coverage") == 1.0,
        "policy_decision_coverage": audit.get("policy_decision_coverage") is True,
    ...
    result = {"schema": "frp-emulator-release-gate/v4", "checks": checks, "failed": failed, "overall_status": "PASS" if not failed else "FAIL"}
